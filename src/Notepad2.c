...
HWND InitInstance(HINSTANCE hInstance, LPSTR pszCmdLine, int nCmdShow)
{
    ...
    else if (flagDefaultPos == 2 || flagDefaultPos == 3 || wi.x == CW_USEDEFAULT || wi.y == CW_USEDEFAULT || wi.cx == CW_USEDEFAULT || wi.cy == CW_USEDEFAULT) {
...
LRESULT MsgCreate(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if (hwndEdit == NULL || hwndEditFrame == NULL || hwndStatus == NULL || hwndToolbar == NULL || hwndReBar == NULL)
        return (-1);
...
void MsgInitMenu(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    EnableCmd(hmenu, IDM_EDIT_SORTLINES,
        SendMessage(hwndEdit, SCI_LINEFROMPOSITION, (WPARAM)SendMessage(hwndEdit, SCI_GETSELECTIONEND, 0, 0), 0) - SendMessage(hwndEdit, SCI_LINEFROMPOSITION, (WPARAM)SendMessage(hwndEdit, SCI_GETSELECTIONSTART, 0, 0), 0) >= 1);
...
LRESULT MsgCommand(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_EDIT_SWAP:
        if (SendMessage(hwndEdit, SCI_GETSELECTIONEND, 0, 0) - SendMessage(hwndEdit, SCI_GETSELECTIONSTART, 0, 0) == 0) {
            int iNewPos;
    ...
    case CMD_WEBACTION2: {
        ...
        char mszSelection[SEL_MAXLEN] = { 0 };
        DWORD cchSelection;
        char* lpsz;
        LPWSTR lpszCommand;
        LPWSTR lpszArgs;
        ...
        if (bCmdEnabled) {
            ...
            cchSelection = (int)SendMessage(hwndEdit, SCI_GETSELECTIONEND, 0, 0) - (int)SendMessage(hwndEdit, SCI_GETSELECTIONSTART, 0, 0);

            if (cchSelection > 0 && cchSelection < SEL_MAXLEN && SendMessage(hwndEdit, SCI_GETSELTEXT, 0, 0) < COUNTOF(mszSelection)) {
                ...
                if (lstrlenA(mszSelection)) {
                    ...
                    lpszCommand = GlobalAlloc(GPTR, sizeof(WCHAR) * (SEL_MAXLEN + COUNTOF(szCmdTemplate) + MAX_PATH + 32));
    ...
    case IDM_EDIT_SAVEFIND: {
        ...
        if (cchSelection > 0 && cchSelection < SEL_MAXLEN && SendMessage(hwndEdit, SCI_GETSELTEXT, 0, 0) < SEL_MAXLEN) {
            char mszSelection[SEL_MAXLEN];
            char* lpsz;
    ...
    case IDC_EDIT:
        ...
        case SCN_UPDATEUI:
            ...
            if (scn->updated & ~(SC_UPDATE_V_SCROLL | SC_UPDATE_H_SCROLL)) {
                ...
                if (SendMessage(hwndEdit, SCI_GETSELECTIONS, 0, 0) > 1 && SendMessage(hwndEdit, SCI_GETSELECTIONMODE, 0, 0) != SC_SEL_RECTANGLE) {
                    int iCurPos = (int)SendMessage(hwndEdit, SCI_GETCURRENTPOS, 0, 0);
                ...
                if (/*iLexer == SCLEX_HTML || iLexer == SCLEX_XML*/ 1) {
                    ...
                    if (iSize >= 3) {
                        ...
                        if (tchBuf[iSize - 2] != '/') {
                            ...
                            if (cchIns > 3 && lstrcmpiA(tchIns, "</base>") && lstrcmpiA(tchIns, "</bgsound>") && lstrcmpiA(tchIns, "</br>") && lstrcmpiA(tchIns, "</embed>") && lstrcmpiA(tchIns, "</hr>") && lstrcmpiA(tchIns, "</img>") && lstrcmpiA(tchIns, "</input>") && lstrcmpiA(tchIns, "</link>") && lstrcmpiA(tchIns, "</meta>")) {
                                SendMessage(hwndEdit, SCI_BEGINUNDOACTION, 0, 0);
...
void ParseCommandLine()
{
    ...
    while (bContinue && ExtractFirstArgument(lp3, lp1, lp2)) {
        ...
        if (!bIsFileArg && lstrcmp(lp1, L"+") == 0) {
            ...
            else if (lstrcmpi(lp1, L"UTF8SIG") == 0 || lstrcmpi(lp1, L"UTF-8SIG") == 0 || lstrcmpi(lp1, L"UTF8SIGNATURE") == 0 || lstrcmpi(lp1, L"UTF-8SIGNATURE") == 0 || lstrcmpi(lp1, L"UTF8-SIGNATURE") == 0 || lstrcmpi(lp1, L"UTF-8-SIGNATURE") == 0)
        ...
        // pathname
            ...
            if (!PathIsRelative(lpFileArg) && !PathIsUNC(lpFileArg) && PathGetDriveNumber(lpFileArg) == -1 /*&& PathGetDriveNumber(g_wchWorkingDirectory) != -1*/) {
...
void CALLBACK WatchTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
    if (bRunningWatch) {
        ...
        // Check Change Notification Handle
            ...
            if (CompareFileTime(&fdCurFile.ftLastWriteTime, &fdUpdated.ftLastWriteTime) != 0 || fdCurFile.nFileSizeLow != fdUpdated.nFileSizeLow || fdCurFile.nFileSizeHigh != fdUpdated.nFileSizeHigh) {
